...
class reporting2:
    ...
    def report(self):
        ...
        for row in self.recupere_events(id_routers):
            #if database_id is not None and str(row.get("database_id")) != str(database_id):
                #continue
    ...
    def _process_batch(self, rows_batch, database_id=61):
        ...
        group_cols = [
            "database_id","segmentId","adv_id","id_routers",
            "tag_id","brand","age_range","gender","main_isp","optimized"
        ]
        ...
        df_grouped["unsubs"] = df_grouped["removals"]
        df_grouped['updated_at']=datetime.now()
        columns_final = [
            "database_id","segmentId","brand","tag_id","adv_id","id_routers","sends","opens","openers",
            "clicks","clickers","unsubs","age_range","gender","main_isp","ca","date_shedule","optimized","updated_at"
        ]
        ...
        def prepare_for_clickhouse(df_grouped):
            ...
            int_cols = [
                "database_id", "segmentId", "tag_id", "adv_id","sends", "opens", "openers", "clicks", "clickers", "unsubs","ca"]
            for col in int_cols:
                df_grouped[col] = df_grouped[col].fillna(0).astype(np.uint32)
            df_grouped["brand"] = df_grouped["brand"].astype("string").fillna("")

            str_cols = ["id_routers","age_range","gender","main_isp","optimized"]
            for col in str_cols:
